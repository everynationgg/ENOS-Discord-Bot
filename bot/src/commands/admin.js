...
const { loadBirthdayQueue, checkUpcomingBirthdayAlerts } = require('../modules/social/birthdays');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub.setName('run-birthday-scan').setDescription('Manually trigger the birthday queue scan & admin alerts')
    )
    .addSubcommand(sub =>
  ...
  async execute(interaction, client) {
    ...
    if (sub === 'run-birthday-scan') {
      ...
      const result = await checkUpcomingBirthdayAlerts(client);
      return interaction.editReply(
        `✅ **Birthday queue scan complete!**\n` +
        `• Synced upcoming member birthdays into queue.\n` +
        `• Admin alerts dispatched: **${result?.alertsSent ?? 0}**\n` +
        `• Review or approve custom greetings in the ENOS Dashboard under Social ➜ Birthday Queue.`
      );
    }
